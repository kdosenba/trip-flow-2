...
const DARK_NEON_PRESET: any = {
  ...initialPaperGlobeStyle,
  "name": "Dark Neon Cyberpunk",
  "layers": initialPaperGlobeStyle.layers.map((layer: any) => {
    ...
    if (layer.id === "hills") {
      return {
        ...layer,
        "paint": { "hillshade-shadow-color": "#110c1a", "hillshade-exaggeration": 0.6 }
      };
    }
    ...
    if (layer.id === "admin_country") {
      return {
        ...layer,
        "paint": { "line-color": "#8b5cf6", "line-width": 1.5, "line-dasharray": [1, 2] }
      };
    }
    if (layer.id === "road_primary_ground_top") {
      return {
        ...layer,
        "paint": { "line-color": "#ec4899", "line-width": { "base": 1.2, "stops": [[5, 0.5], [12, 3], [16, 7], [20, 16]] } }
      };
    }
    if (layer.id === "road_motorway_ground_top") {
      return {
        ...layer,
        "paint": { "line-color": "#3b82f6", "line-width": { "base": 1.2, "stops": [[5, 0.5], [12, 3.5], [16, 10], [20, 22]] } }
      };
    }
...
const EMERALD_FOREST_PRESET: any = {
  ...initialPaperGlobeStyle,
  ...
  "layers": initialPaperGlobeStyle.layers.map((layer: any) => {
    ...
    if (layer.id === "hills") {
      return {
        ...layer,
        "paint": { "hillshade-shadow-color": "#1b5e20", "hillshade-exaggeration": 0.4 }
      };
    }
    if (layer.id === "landcover_wood") {
      return {
    ...
    if (layer.id === "road_primary_ground_top") {
      return {
        ...layer,
        "paint": { "line-color": "#ff8a65", "line-width": { "base": 1.2, "stops": [[5, 0.2], [12, 2.5], [16, 7], [20, 16]] } }
      };
    }
    if (layer.id === "road_motorway_ground_top") {
      return {
        ...layer,
        "paint": { "line-color": "#ffb74d", "line-width": { "base": 1.2, "stops": [[5, 0.5], [12, 3], [16, 10], [20, 22]] } }
      };
    }
    if (layer.id.startsWith("label_")) {
      return {
...
const FLIGHT_DESTINATIONS = [
  { name: "New York City", center: [-73.97, 40.71], zoom: 13, pitch: 65, bearing: 0 },
  { name: "Swiss Alps (Matterhorn)", center: [7.6586, 45.9763], zoom: 12, pitch: 75, bearing: 120 },
  { name: "Grand Canyon", center: [-112.1129, 36.1069], zoom: 11, pitch: 65, bearing: -30 },
  { name: "Mount Everest", center: [86.9250, 27.9881], zoom: 12.5, pitch: 70, bearing: 45 }
];
...
export default function GlobePage() {
  ...
  const [currentStyle, setCurrentStyle] = useState<any>(initialPaperGlobeStyle);
  ...
  // Synchronize style state with raw text editor initially and on update
  useEffect(() => {
  ...
  const handleExaggerationSlider = (val: number) => {
    ...
    setCurrentStyle((prev: any) =>
      produce(prev, (draft: any) => {
        if (!draft.terrain) draft.terrain = {};
        draft.terrain.exaggeration = val;
      })
    );
  };
  ...
  const applyPreset = (presetKey: string) => {
    ...
    let targetStyle: any = initialPaperGlobeStyle;
    ...
    <div style={{
      ...
      <section style={{
        ...
        <header style={{
          ...
          <div style={{ display: "flex", alignItems: "center", gap: "0.5rem" }}>
            <span style={{ fontSize: "0.75rem", color: "#a1a1aa" }}>Preset:</span>
            <select
        ...
        {/* Editor Render Panel */}
        <div style={{ flex: 1, overflow: "auto", padding: "1.25rem", background: "#0c0c0f" }}>
          {editorTab === "tree" ? (
      ...
      <section style={{
        ...
        <div style={{
          ...
          <div style={{ display: "flex", gap: "0.4rem", flexWrap: "wrap", maxWidth: "420px" }}>
        ...
        <div style={{
          ...
          <div style={{ display: "flex", flexDirection: "column", gap: "0.25rem" }}>
            <span style={{ fontSize: "0.65rem", color: "#a1a1aa", textTransform: "uppercase" }}>Auto Orbit</span>
            <button
              onClick={() => setIsSpinning(!isSpinning)}
              style={{
                background: isSpinning ? "linear-gradient(135deg, #a855f7, #6b21a8)" : "#27272a",
                color: "#ffffff",
                border: "none",
                borderRadius: "8px",
                padding: "8px 16px",
                ...
                boxShadow: isSpinning ? "0 0 12px rgba(168, 85, 247, 0.6)" : "none"
              }}
            >
              <span style={{
          ...
          <div style={{ display: "flex", flexDirection: "column", gap: "0.25rem" }}>
            <span style={{ fontSize: "0.65rem", color: "#a1a1aa", textTransform: "uppercase" }}>Terrain 3D</span>
            <button
              onClick={toggleTerrain}
              style={{
                background: terrainOn ? "linear-gradient(135deg, #3b82f6, #1d4ed8)" : "#27272a",
                color: "#ffffff",
                border: "none",
                borderRadius: "8px",
                padding: "8px 16px",
                ...
                cursor: "pointer",
                transition: "all 0.3s cubic-bezier(0.4, 0, 0.2, 1)",
                boxShadow: terrainOn ? "0 0 12px rgba(59, 130, 246, 0.6)" : "none"
              }}
            >
          ...
          <div style={{ flex: 1, display: "flex", flexDirection: "column", gap: "0.25rem" }}>
            <div style={{ display: "flex", justifyContent: "space-between", fontSize: "0.65rem", color: "#a1a1aa" }}>
              <span>MOUNTAIN RELIEF</span>
              <span style={{ color: "#3b82f6", fontWeight: "bold" }}>{exaggeration.toFixed(1)}x</span>
            </div>
            <input
              type="range"
              min="0"
              ...
              value={exaggeration}
              disabled={!terrainOn}
              onChange={(e) => handleExaggerationSlider(parseFloat(e.target.value))}
              style={{
                width: "100%",
                accentColor: "#3b82f6",
