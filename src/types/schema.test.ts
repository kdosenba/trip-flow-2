...
export const runSchemaTests = async () => {
  ...
  try {
    ...
    console.log('✅ Happy Path: Successfully parsed graph with cost, budget, target dates, and default client context.');
    
    // Assert clientContext defaults loaded
    if (!graph.clientContext || typeof graph.clientContext.language !== 'string') {
      throw new Error('Default clientContext language was not populated correctly');
    }
    console.log(`ℹ️ Resolved Client defaults -> Language: ${graph.clientContext.language}`);
  } catch (err) {
  ...
  try {
    ...
    console.error('❌ Invalid range validation: Allowed target end date to be before start date incorrectly.');
    throw new Error('Allowed invalid target range');
  } catch (err) {
    console.log('✅ Target date range constraints (end >= start) correctly rejected invalid range.');
  }
  ...
  try {
    ...
    console.error('❌ WhereAmILocation validation: Allowed missing country_name.');
    throw new Error('Allowed invalid WhereAmI location missing country_name');
  } catch (err) {
    console.log('✅ WhereAmILocation constraints correctly rejected missing country_name.');
  }

  // 6. Travelpayouts normalization adapter verification
  try {
    const originalFetch = globalThis.fetch;
    const mockFetch = (responseObj: any, ok = true) => {
    ...
    if (normalized.coordinates.lat !== 49.0097 || normalized.coordinates.lng !== 2.5479) {
      throw new Error('Coordinates split was parsed incorrectly');
    }
    console.log('✅ Travelpayouts Adapter: Happy path coordinate transformation verified.');
    ...
    console.log('✅ Travelpayouts Adapter: Optional field (iata) omission handled correctly.');
    ...
    try {
      ...
      console.error('❌ Travelpayouts Adapter: Allowed invalid coordinate format (missing colon).');
      throw new Error('Allowed invalid coordinates format');
    } catch (e) {
      console.log('✅ Travelpayouts Adapter: Correctly rejected malformed coordinate format (missing colon).');
    }
    ...
    try {
      await fetchWhereAmI('en-US');
      console.error('❌ Travelpayouts Adapter: Allowed non-numeric coordinate coordinates.');
      throw new Error('Allowed invalid coordinate values');
    } catch (e) {
      console.log('✅ Travelpayouts Adapter: Correctly rejected non-numeric coordinates.');
    }
    ...
    try {
      await fetchWhereAmI('en-US');
      console.error('❌ Travelpayouts Adapter: Allowed payload missing name field.');
      throw new Error('Allowed missing required fields');
    } catch (e) {
      console.log('✅ Travelpayouts Adapter: Correctly rejected payloads missing required API properties.');
    }
    ...
    try {
      ...
      if (loc && loc.coordinates) {
        console.log(`✅ fetchWhereAmI Integration: Successfully resolved geolocated location -> ${loc.name}, ${loc.country_name}`);
      }
    } catch (e) {
      // Gracefully allow network failure/offline/rate limit blocks in unit tests
      console.log('ℹ️ fetchWhereAmI Integration: Geolocation query ran (network/rate-limit handled cleanly):', (e as Error).message);
    }
  ...
  try {
    if (getCurrencyForCountry('FR') !== 'EUR' || getCurrencyForCountry('JP') !== 'JPY' || getCurrencyForCountry('unknown') !== 'USD') {
      throw new Error('Country-to-currency resolver mapping is incorrect');
    }
    console.log('✅ Dynamic Currency Resolution: Country lookup tables verified successfully.');
  } catch (err) {
