...
export const generateCityHubId = (cityName: string): CityHubId => {
  const slug = cityName
    ...
    .replace(/[^a-z0-9]+/g, '_')
    .replace(/^_+|_+$/g, '');
  return CityHubIdSchema.parse(slug ? `hub_${slug}_${shortId()}` : `hub_${shortId()}`);
};
...
export const generateTransitId = (fromCityId: string, toCityId: string): TransitId => {
...
export const generateSuggestionId = (title: string): SuggestionId => {
  const slug = title
    ...
    .replace(/[^a-z0-9]+/g, '_')
    .replace(/^_+|_+$/g, '');
  return SuggestionIdSchema.parse(slug ? `suggest_${slug}_${shortId()}` : `suggest_${shortId()}`);
};
