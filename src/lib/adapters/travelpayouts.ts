 ...
 */
interface TravelpayoutsWhereAmIResponse {
  iata?: string;
  name?: string;
  country_name?: string;
  country_code?: string;
...
export const fetchWhereAmI = async (
  language: string,
): Promise<WhereAmILocation> => {
  ...
  if (!response.ok) {
    throw new Error(
      `Failed to fetch geolocation from Travelpayouts: ${response.statusText}`,
    );
  }

  const rawData = (await response.json()) as TravelpayoutsWhereAmIResponse;
  ...
  if (typeof coordinatesStr !== "string") {
    throw new Error(
      "Invalid response from Travelpayouts: Missing coordinates string",
    );
  }
  ...
  if (parts.length !== 2) {
    throw new Error(
      `Invalid coordinate format: expected "lat:lng" but got "${coordinatesStr}"`,
    );
  }
